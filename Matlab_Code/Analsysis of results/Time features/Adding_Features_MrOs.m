...
load('mrOz__Features_1')

tableadd=readtable('mros-visit1-dataset-0.6.0.csv');
%%
for i = 1:length(Features)
   
    for k = 1:size(tableadd,1)
        
        if strcmp(Features(i).nsrrid,tableadd.nsrrid(k))
           
            Features(i).AHI=tableadd.poahi3a(k);
            Features(i).Coffee=tableadd.poxcoff(k);
