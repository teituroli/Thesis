...
function Features=ToSherlock_M_bands(Info,Feature_path,Annotation_path,StateInv,Iterations,new_or_old,issherlock,Cohort)
...
for w = 1:length(EEGNames)
    PowerSpects.(Channels{zz})(size(Info,1)).(EEGNames{w})=[];
end
end
...
for i = 1:Iterations
    ...
    parfor p_num=1:size(Info,1)%sum(IDsToKeep_PI)% pnum11 is t4
        HDR=[];
        SSC=[];
        data_raw=[];
        Events=[];
        if isempty(Features(p_num).ID)
            ...
            if ~isempty(HDR)
                try
                    ...
                    [tmpfeatures,Pspects]=Features_M_bands(data_raw,HDR,SSC,Info(p_num,:),false,Events);
...
%                     end
                    try
                        Pspects.nsrrid=Info.nsrrid(p_num);
                    catch
                        try
                            Pspects.nsrrid=Info.NSRRID(p_num);
                        catch
...
%end
%%
function parsave(Pspects,Cohort)
try
    save([Cohort '_PSpect_' num2str(Pspects.nsrrid)],"Pspects")
catch
    save([Cohort '_PSpect_' Pspects.nsrrid{:}],"Pspects")
end
end